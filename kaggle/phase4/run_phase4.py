"""
Kaggle kernel script for Phase 4 mechanistic analysis on T4 GPU.

Collects residual-stream activations for clean, CAA-injected, and neuron-
suppressed forward passes, then runs CPU-side geometry and linear probe analysis.
All output artifacts are written to /kaggle/working/.
"""
...
def run_phase4():
    ...
    config_path = prepare_phase4_config()
    return main(["--config", config_path, "--backend", "transformers"])


def prepare_phase4_config():
    import yaml
...
if __name__ == "__main__":
    ...
    print("=== Running Phase 4: Mechanistic Geometry + Linear Probing ===")
    code = run_phase4()

    print(f"\n=== Phase 4 finished with exit code {code} ===")
    print("Artifacts written to /kaggle/working/artifacts/phase4/")
    sys.exit(code)
